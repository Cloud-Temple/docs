...
La gestion des accès dans la Console repose sur deux niveaux imbriqués :

- l'__[organisation](#organisations)__ : l'entité contractuelle. Elle porte les comptes utilisateurs, la configuration d'authentification et l'ensemble de vos tenants ;
- le __[tenant](#tenant)__ : un regroupement de ressources au sein de l'organisation. Il porte les ressources techniques et les permissions des utilisateurs.

Entre les deux se trouve un __périmètre commun__ : l'identité de l'utilisateur. Un compte est créé une seule fois dans l'organisation, puis utilisé dans tous les tenants auxquels il a accès.

### Qui gère quoi

| Objet géré                                        | Organisation                                      | Tenant                                        |
| ------------------------------------------------- | ------------------------------------------------- | --------------------------------------------- |
| Contrat et facturation                            | Oui                                               | Rapport de consommation par tenant            |
| Mécanisme d'authentification (local, SSO)          | Oui                                               | Non (hérité de l'organisation)                |
| Cycle de vie des comptes (invitation, suppression) | Oui                                               | Non (les comptes sont globaux à l'organisation) |
| __Permissions des utilisateurs__                   | __Non__                                           | __Oui, tenant par tenant__                    |
| __Désignation d'un propriétaire__                  | __Oui__ (propriétaire de l'organisation)          | __Oui__ (propriétaire du tenant)              |
| Ressources techniques (calcul, stockage, réseau)   | Non                                               | Oui                                           |
| Adresses IP autorisées                             | Oui                                               | Non (commun à l'organisation)                 |

:::info[À retenir]
Les permissions ne se configurent __jamais__ au niveau de l'organisation : elles sont définies tenant par tenant. Le niveau organisation permet en revanche de désigner un utilisateur comme __propriétaire de l'organisation__.
:::

## Périmètre commun : le compte utilisateur
...
Les informations d'identification sont globales à votre [Organisation](#organisations) : un utilisateur dispose d'un seul compte, quel que soit le nombre de tenants dans lesquels il intervient.

*__Nota :__ [La fédération d'identité se gère au niveau de l'organisation](#mécanismes-dauthentification)*

Les actions décrites ci-dessous relèvent du compte lui-même, indépendamment du tenant dans lequel vous êtes connecté.
...
### Ce qui se gère au niveau de l'organisation

Le périmètre organisation couvre :

- le __mécanisme d'authentification__ commun à tous les comptes,
...
- les __adresses IP autorisées__ à joindre la Console.
...
Par défaut, __un utilisateur nouvellement créé n'a aucun droit__. Il est donc nécessaire de lui octroyer ensuite les permissions nécessaires à son activité, __dans chaque tenant__ où il doit intervenir.

### Gestion des propriétaires de l'organisation

Le __propriétaire de l'organisation__ est le seul rôle attribuable au niveau de l'organisation. Il est distinct du [propriétaire de tenant](#gestion-des-propriétaires-sur-un-tenant).

- La désignation se fait depuis la page __'Utilisateurs'__ du volet Administration, via le menu d'action de la ligne de l'utilisateur.
- Ce rôle donne accès à la __vue organisation__ : suivi contractuel, liste des tenants, gestion des comptes.
- Il ne s'agit pas d'une permission granulaire : il n'y a aucune permission de type `read` / `write` à attribuer au niveau de l'organisation.
- Un utilisateur __'Propriétaire'__ ne peut pas être supprimé.
...
Un tenant est créé __sans aucun produit activé__ : vous choisissez à la création, puis à tout moment via __'Modifier les produits'__, ceux que vous souhaitez y ouvrir. Rien n'est activé d'office.

Les produits d'infrastructure historiques (IaaS) restent, eux, commandés par référence :
...
### Ce qui se gère au niveau du tenant

Le périmètre tenant couvre :

- les __ressources techniques__ commandées et déployées,
- les __permissions des utilisateurs__ sur ces ressources,
- la __désignation des propriétaires du tenant__,
- le __suivi de consommation__ des ressources.

Les permissions des utilisateurs sont à définir dans chaque tenant. Un compte peut ainsi être autorisé à commander des ressources dans un tenant, mais pas dans un autre. Chaque organisation doit donc bien réfléchir aux tenants souhaités : ce point est généralement abordé en atelier d'initialisation, au moment de la création de l'organisation.

### Affectation des permissions dans un tenant

La gestion des droits d'un utilisateur se fait depuis la page des utilisateurs, __tenant par tenant__.
...
La configuration des permissions est à refaire pour chaque [Tenant](#tenant) de l'[Organisation](#organisations) dans lequel l'utilisateur doit intervenir : il n'existe pas d'attribution globale au niveau de l'organisation.
...
Le propriétaire de tenant est un rôle distinct du [propriétaire de l'organisation](#gestion-des-propriétaires-de-lorganisation) : il porte la responsabilité d'un périmètre de ressources, pas de la relation contractuelle.
...
### Autorisation d'accès à la Console : IP autorisées
...
Cette liste relève du périmètre __organisation__ : elle est commune à tous vos tenants et se gère depuis __'Administration > Accès'__.
...
La Console permet une gestion fine des droits des utilisateurs d'une organisation, avec une __ségrégation par tenant__.
...
Par la suite, le droit __'iam_write'__ permet à un compte d'administrer les permissions des autres utilisateurs, dans le tenant où ce droit lui a été accordé.
...
| backup_iaas_opensource_read                   | Offre OpenIaaS - Gestion des ressources de type backup - consultation                                                         |
| backup_iaas_opensource_write                  | Offre OpenIaaS - Gestion des ressources de type backup - modification                                                         |
| backup_iaas_spp_read                          | Offre Vmware - Gestion des ressources de type backup - consultation                                                           |
| backup_iaas_spp_write                         | Offre Vmware - Gestion des ressources de type backup - modification                                                           |
...
| billing_read                                  | Consultation des dashboards de consommations                                                                                  |
| vpc_read                                      | Offre VPC - Consultation des ressources réseaux type VPC                                                                      |
| vpc_write                                     | Offre VPC - Gestion des ressources réseaux type VPC                                                                           |
| public_cloud_vm_instances_management          | Offre VM Instances - Gestion des machines virtuelles                                                                          |
| public_cloud_vm_instances_read                | Offre VM Instances - Consultation des machines virtuelles                                                                     |
| public_cloud_vm_instances_console_access      | Offre VM Instances - Ouverture de la console des machines virtuelles                                                          |
