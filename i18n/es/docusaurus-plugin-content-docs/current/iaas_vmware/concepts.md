...
- Copias de seguridad: __IBM Spectrum Protect Plus__, para la orquestación y el almacenamiento de copias de seguridad.

Esta arquitectura se basa en el modelo __VersaStack__, una alianza entre Cisco e IBM, garantizando una compatibilidad amplia con los principales fabricantes de software.
...
When using a virtualization offering, a hypervisor cluster must consist exclusively of blades of the same type (it is not possible to mix different blade types within the same cluster).
...
- __(4)__ The actual GPU offering is continuously evolving. As of May 1, 2024, the offering is based on NVIDIA LOVELACE L40S GPUs. By default, the PERF4 blade is delivered with two 48 GB L40S cards. Contact support for further details if needed.

The computing offering availability is 99.99%, calculated monthly, including maintenance windows. Eligibility for SLA non-compliance requires the creation of an incident ticket. You must also have at least two hosts per cluster and enable the __High Availability__ (HA) feature.  
...
- __Compartición entre clústeres y zonas de disponibilidad (AZ)__:
  - Los VLANs pueden compartirse entre diferentes AZ y distintos clústeres que pertenecen al mismo inquilino.
...
1. __Flexibilidad__ : Los VLAN pueden configurarse para adaptarse a entornos multi-clúster y multi-inquilinos.
...
The __IBM SVC__ technology enables delivering the required performance levels for our clients' environments, thanks to the large amount of memory cache built into the controllers and the ability to distribute a server's total IOPS across multiple SANs.
...
2. __Espacio libre para instantáneas de copia de seguridad__: El servicio de copia de seguridad utiliza instantáneas instantáneas (snapshots). Por lo tanto, siempre debe tener suficiente espacio libre para permitir la creación de una instantánea durante la copia de seguridad de una máquina virtual. El tamaño de la instantánea depende del volumen de escritura de la máquina virtual y del tiempo necesario para realizar la copia de seguridad. En general, se recomienda mantener al menos un 10 % de espacio libre para esta operación.
...
- La replicación __sincrónica__ (o __'Metro Mirror'__): *La función __'Metro Mirror'__ es un tipo de copia remota que crea una copia sincrónica de los datos de un volumen primario hacia un volumen secundario. Con copias sincrónicas, las aplicaciones anfitrionas escriben en el volumen primario, pero no reciben confirmación de que la operación de escritura ha finalizado hasta que los datos se hayan escrito en el volumen secundario. Esto garantiza que ambos volúmenes contengan datos idénticos cuando finaliza la operación de copia. Tras finalizar la operación de copia inicial, la función Metro Mirror mantiene permanentemente una copia completamente sincronizada de los datos de origen en el sitio objetivo. __Desde el 1 de enero de 2024, la función 'Metro Mirror' ya no se comercializa.__*

Se define entonces un sitio denominado "activo" o "principal" y un sitio "pasivo" o "en espera". El plan de recuperación de actividad se activa en caso de siniestro o dentro del marco de una prueba del PRA. A continuación, el sitio pasivo asume el control del sitio activo.
...
Esta oferta permite obtener un __RPO de 15 minutos__ y un __RTO inferior a 4 horas__. Permite reanudar las operaciones mucho más rápidamente que la implementación de una restauración de copia de seguridad.

En un volumen de almacenamiento con replicación asíncrona (__Global Mirror__), los controladores de virtualización SAN de ambas zonas de disponibilidad trabajan conjuntamente para realizar las operaciones de escritura en ambos sitios. El sitio principal no espera la confirmación de escritura del sitio secundario.
...
4. Envía la __confirmación__ al hipervisor que emitió la solicitud,
...
| SLA       | Descripción                                                                                                                                       |
|-----------|---------------------------------------------------------------------------------------------------------------------------------------------------|
| RPO 15 min | En caso de siniestro en el centro de datos principal, la cantidad máxima de datos perdidos corresponde a lo más a los 15 últimos minutos de escritura |
...
En caso de activación del PRA, el equipo de Cloud Temple realiza una operación de presentación de la LUN __'Global Mirror'__ a los hipervisores remotos para que puedan acceder a los datos. La implementación de esta solución requiere, por tanto, haber reservado en el sitio de 'standby' los recursos de cálculo y RAM necesarios para reanudar la actividad en caso de siniestro.
...
- *Dado que la oferta es asíncrona, en caso de siniestro es posible que algunas operaciones de disco no se hayan escrito en el sitio remoto. Por tanto, puede existir un riesgo de coherencia de datos, mitigado en el análisis de riesgos del plan de recuperación de actividad.*
...
- *El cálculo y la memoria en el sitio de recuperación pueden reducirse para optimizar los costos si se acepta una situación degradada para el negocio durante la ejecución del plan de recuperación de actividad.*
...
Puede ser controlada a través de la interfaz gráfica de la consola Shiva o mediante las APIs asociadas.
...
- *El almacenamiento no utiliza la tecnología VMware vSAN, sino únicamente SANs IBM en fibra canal de 32 G.*  
- *No se implementa ninguna forma de optimización oculta (compresión, deduplicación, ...).*

### Definición de un cluster de nodos de cálculo ('Cpool')

El __'Cpool'__ es un agrupamiento de hipervisores VMware ESXi, también conocido como *'cluster ESX'*.

Los hosts incluidos en un __'Cpool'* pertenecen todos al mismo inquilino y a la misma zona de disponibilidad (AZ). Necesariamente deben tener la misma clase:  
__no es posible mezclar modelos diferentes de nodos de cálculo dentro de un mismo cluster__.

Dado que todos los nodos de cálculo se entregan con la memoria física máxima, se aplica una limitación de uso de RAM a nivel del cluster mediante software para asegurarse de que coincida con la RAM facturada.

Por defecto, cada nodo dispone de 128 GB de memoria activada dentro del __'Cpool'*.

Un __'Cpool'* puede contener como máximo 32 hipervisores. Si se supera este límite, será necesario crear un segundo cluster.

El almacenamiento puede ser compartido entre los __'Cpool'*
...
Each compute node comes with 128 GB of memory enabled at the __'Cpool'__ level, but physically has access to all available memory.
...
You can extend it up to a maximum of 3 x 384 GB = 1,152 GB of memory.
    ...
    Maximum memory for a 'Cpool' = number of hosts × total physical memory available per compute node
...
En un __'Cpool'__ VMware, estas reglas se utilizan frecuentemente para gestionar el comportamiento de las máquinas virtuales con vMotion.  
...
La replicación asíncrona de sus máquinas virtuales es un mecanismo que consiste en enviar, desde el hipervisor origen, las operaciones de escritura al sitio de respaldo a intervalos de tiempo regulares.
...
esta solución presenta los mismos inconvenientes, especialmente la sensibilidad al volumen de escrituras de la máquina virtual,  
dado que el proceso de instantánea es un mecanismo recursivo para cerrar la instantánea.
...
| SLA             | Descripción                                                                                                                                               |
|-----------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------|
| RPO de 1H a 24H | En caso de siniestro en el centro de datos principal, la cantidad máxima de datos perdidos será la correspondiente a la última sincronización de escrituras al sitio de respaldo. |
...
Cloud Temple ofrece una __arquitectura nativa de copia de seguridad cruzada e inmutable__ (es obligatoria en la calificación secnumcloud francesa).
...
- Presentación e inicio instantáneo de una máquina virtual desde la infraestructura de almacenamiento masivo y recarga en caliente en los SAN de producción,
...
| Referencia                                               | Unidad | SKU                            |
| ------------------------------------------------------- | ------ | ------------------------------ |
| SAQUEGARDA - Acceso al servicio IBM Spectrum Protect Plus | 1 VM   | csp:(region):iaas:backup:vm:v1 |
...
- __Global Mirror__: Las máquinas virtuales cifradas **no son compatibles** con la replicación Global Mirror
- __Restauración__: Las copias de seguridad de máquinas virtuales cifradas conservan su protección criptográfica
- __Exportación__: La exportación de máquinas virtuales cifradas requiere procedimientos específicos
