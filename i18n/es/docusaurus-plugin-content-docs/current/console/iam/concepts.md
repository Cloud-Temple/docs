...
La creación de una cuenta de usuario en su organización se realiza mediante invitación. Para invitar a un usuario a una [Organización](#organisations), vaya al menú __'Administración'__ en el lado izquierdo de su pantalla, en la barra verde, y luego al submenú __'Usuarios'__.
...
To perform a user's re-registration, go to the __'User'__ tab in the Administration panel, located at the bottom-left of the screen.
...
__Warning__: Make sure you are the original requester of the profile update. Please report any requests that do not come from you via a support ticket.
...
La configuración se realiza para cada inquilino [Tenant](#tenant).
...
Está disponible en el perfil del usuario, en la pestaña "Mis suscripciones":
...
### Permis disponibles para los usuarios de su organización

Cuando se crea un usuario, no posee ningún permiso por defecto. Cada permiso se asigna individualmente y funciona de forma unitaria, sin solapamiento con otros permisos. Se aplican en conjunto, lo que significa que un usuario debe tener todos los permisos necesarios para realizar una acción específica.
...
- Permisos de tipo “console_access”: permiten conexiones tipo PMAD sobre recursos.
...
La organización está vinculada a su __cuenta patrocinadora__ y al __contrato Cloud Temple asociado__. Representa su entidad (empresa, departamento, equipo, ...) que ostenta la relación contractual entre Cloud Temple y usted.
...
- Define la __configuración global del mecanismo de autenticación__: la autenticación puede ser local en la consola Shiva o remota a través de un servicio de federación de identidades,
- Gestiona todo el conjunto de __cuentas de usuarios__,
...
Los permisos de los usuarios deben definirse en cada inquilino. Por tanto, cada organización debe reflexionar cuidadosamente sobre los inquilinos deseados. Este punto se aborda normalmente en un taller de inicialización, al momento de crear la organización.
...
Note: *To remove an authorized IP, submit a support request via the Cloud Temple console.*
...
En el menú de la consola, haga clic en "Informe de consumo" y seleccione el período de tiempo deseado. Así podrá visualizar detalladamente el consumo de recursos de nube durante el período definido, lo que le permitirá analizar el uso de los servicios y optimizar su gestión en consecuencia:
