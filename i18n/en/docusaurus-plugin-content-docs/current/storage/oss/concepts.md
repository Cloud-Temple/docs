...
The Cloud Temple object storage offering is based on the __Elastic Cloud Storage (ECS)__ technology from Dell, recognized for its high performance and adherence to industry standards. With a __97% compatibility with the AWS S3 protocol__, this solution ensures:
...
| ------------------------ | --------------------------------------- | --------------------------------- |
| __SSE-ECS__             | Simplified and transparent management   | Less control over the keys        |
| __SSE-C__               | Full control over the keys              | Key management required           |
...
- Erasure Coding (EC): We default to an EC 12+4 scheme, which splits data into 12 data segments and 4 parity segments. This technique allows data reconstruction even if multiple segments are lost.
...
- Geographic Replication: For additional protection, data is replicated across 3 availability zones, providing resilience against local incidents.
...
It is on this account that the roles and permissions associated with **buckets** are set, allowing precise control over access and the actions allowed for each user or service.
...
*   **Granular Permissions**: The access rights of this account are defined at the level of each bucket via access control lists (ACL). You must explicitly grant permissions (read, write, etc.) on the buckets it needs to access.

#### 2. Global Storage Account (Root)

Each *namespace* (tenant) has a single global storage account, sometimes called the "root" account. This account has extended administrative privileges.

*   **Full Access**: The global account has complete access to all buckets within the namespace, without needing specific permissions assigned. It can perform all possible operations on the entire storage service.
*   **Administrative Usage**: It is primarily intended for configuration and global administration tasks.
...
Due to the constraints associated with the SecNumCloud qualification, the offer is planned to use the '__PathStyle__' method at this time. We are working to have the '__UrlStyle__' method available in Q2 2025.

### Pre-signed requests

Cloud Temple object storage supports __pre-signed requests__, a critical feature that allows generating temporary URLs granting access to specific objects for a limited duration. This feature is particularly useful for securely sharing files with external users without granting them permanent rights or bucket access credentials. Pre-signed requests can be configured with a precise validity period, providing granular control over data access.

### Object Immutability (Object Lock)

Cloud Temple's Object Storage, based on Dell ECS, supports the immutability feature via **Object Lock**, in compliance with the S3 standard. This option allows configuring objects in **WORM (Write Once, Read Many)** mode, thus protecting them against any modification or deletion during a defined period. It is an essential protection for regulatory compliance and defense against ransomware.
