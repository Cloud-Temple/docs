---
title: Concepts - Regions
---

import iaasAz001 from '@site/docs/additional_content/images/iaas_az_001.png';

## Regions

The Cloud Temple infrastructure is designed to ensure business continuity for your applications and platforms.
Within a region, you benefit from level 2 network continuity.

Thus, it is possible to define a technical architecture in the Paris region, using 3 physical sites to enable the deployment of a high-availability platform with quorum.

As of January 2024, the Cloud Temple IaaS platform offers the following regions:

- Region FR1 (Paris)

### FR1 Region

The FR1 region is based on three physical sites in the Paris area:

- FR1 - AZ05 (PAR7S)
- FR1 - AZ06 (TH3S)
- FR1 - AZ07 (DATA4)

<img src={iaasAz001} />

Each site is more than 30 km away from the others in a straight line. On average, there is 80 km of optical distance between each site. The guaranteed latency between sites is less than 4 ms.

### Regional Expansions

The opening of new regions is primarily associated with market opportunities. In January 2024, the opening of a region in Southern Europe and a region in Northern Europe are under consideration.